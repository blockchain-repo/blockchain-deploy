...
#获取该节点主机名
hostaname=`hostname`
echo -e  "$hostaname 硬件信息报告：" >> env_node_$hostaname
...
#获取该节点主机名
hostaname=`hostname`
echo   "$hostaname 节点服务环境安装信息报告：" >> env_node_$hostaname
echo   " " >> env_node_$hostaname
...
if [ -z "$python_3" ];then
    ...
    echo   "python_3：未安装" >> env_node_$hostaname
    echo   " " >>  env_node_$hostaname
else
    echo "Python Has been installed"
    echo   "python_3：已安装" >> env_node_$hostaname
    echo   " " >>  env_node_$hostaname
fi
...
if [ -z "$pip_3" ];then
    ...
    echo   "pip_3：未安装" >> env_node_$hostaname
    echo   " " >>  env_node_$hostaname
else
    echo "pip_3 Has been installed"
    echo   "pip_3：已安装" >> env_node_$hostaname
    echo   " " >>  env_node_$hostaname
fi
...
if [ ! -z "$python_3" ]  && [ ! -z "$collectd_version" ] && [ ! -z "$pip_3" ];then
   echo "ready go!"
    echo   "基本环境安装已ok，请开始下一步操作！" >> env_node_$hostaname
    echo   " " >>  env_node_$hostaname
else
    echo "please install python3 fabric_3 tools"
    echo   "注意：python_3,pip_3或fabric_3未安装，请安装服务基本环境！" >> env_node_$hostaname
    echo   " " >>  env_node_$hostaname
fi
...
#获取该节点主机名
hostaname=`hostname`
echo   "$hostaname 区块链服务安装信息报告：" >> env_node_$hostaname
echo   " " >> env_node_$hostaname

rethinkdb_number=`ps -aux|grep -E "/usr/bin/rethinkdb"|grep -v grep|wc -l`
if [ $rethinkdb_number -eq 0 ];then
    echo   "rethinkdb 进程数为：0" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
else
    echo   "rethinkdb 进程数为：$rethinkdb_number" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
fi

driver_port=`netstat -nlap|grep "LISTEN"|grep rethinkdb|grep "28015"`
if [ -z $driver_port ];then
    echo   "rethinkdb：28015未被使用" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
else
    echo   "rethinkdb：28015已被使用" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
fi
cluster_port=`netstat -nlap|grep "LISTEN"|grep rethinkdb|grep "29015"`
if [ -z $cluster_port ];then
    echo   "rethinkdb：29015未被使用" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
else
    echo   "rethinkdb：29015已被使用" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
fi
http_port=`netstat -nlap|grep "LISTEN"|grep rethinkdb|grep "8080"`
if [ -z $http_port ];then
    echo   "rethinkdb：8080未被使用" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
else
    echo   "rethinkdb：8080已被使用" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
fi
...
if [ $unichain_number -eq 0 ];then
    echo   "unichain 进程数为：0" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
else
    echo   "unichain 进程数为：$unichain_number" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
fi
unichain_port=`netstat -nlap|grep "LISTEN"|grep "9984"`
if [ -z $unichain_port ];then
    echo   "unichain：9984未被使用" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
else
    echo   "unichain：9984已被使用" >> env_node_$hostaname
    echo   " " >> env_node_$hostaname
fi

if [ $rethinkdb_number -gt 0 ] && [ $unichain_number -gt 0 ] && [ ! -z "$driver_port" ]  && [ ! -z "$cluster_port" ] && [ ! -z "$http_port" ] && [ ! -z "$driver_port" ];then
   echo "ready go!"
    echo   "区块链服务安装已ok，请开始下一步操作！" >> env_node_$hostaname
    echo   " " >>  env_node_$hostaname
else
    echo "please install python3 fabric_3 tools"
    echo   "注意：unichain或rethinkdb未安装，请安装服务！" >> env_node_$hostaname
    echo   " " >>  env_node_$hostaname
fi
