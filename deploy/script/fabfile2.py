...
def local_update_apt_pip(apt=True, pip=True):
    with settings(warn_only=True):
        if apt:
            local("sudo cp ../sources/trusty-sources.list /etc/apt/")
            local("echo renew the apt trusty-sources.list")
        if pip:
            if local("test -d ~/.pip").failed:
                local("mkdir ~/.pip")
            local("cp ../sources/pip.conf ~/.pip/")
            local("echo renew the pip.conf")
...
def update_apt_pip(apt=True, pip=True):
    with settings(warn_only=True):
        if apt:
            put("../sources/trusty-sources.list", "/etc/apt/",
                mode=0x0600, use_sudo=True)
            run("echo renew the apt trusty-sources.list")
        if pip:
...
def install_collectd():
    ...
    with settings(warn_only=True):
        ...
        sudo("echo 'deb http://http.debian.net/debian wheezy-backports-sloppy main contrib non-free' | sudo tee /etc/apt/trusty-sources.list.d/backports.list")
        # fixed the GPG Error
...
def install_docker():
    with settings(warn_only=True):
        sudo("echo deb https://apt.dockerproject.org/repo ubuntu-trusty main > /etc/apt/trusty-sources.list.d/docker.list")
